...
class Ion_auth
{
	...
	/**
	 * Check for Link Access for the Groups mapped to the user.
	 *
	 * @return bool
	...
	{		
		...
		$curr_controller = $this->uri->segment ( 1 );
		if (empty ( $curr_controller )) {
			return true;
		}
		//$curr_controller_method=$this->uri->segment(2);
		$menuids = array ();
		foreach ( $user_groups as $group ) {
			$menuids = array_merge ( $menuids, explode ( ',', $group ['menu_id'] ) );
		}

		$menu = implode ( ',', array_unique ( array_filter($menuids )));	
		$links = $this->ion_auth_model->menu_list_links ( $menu );
		$link_found = 0;
		foreach ( $links as $link ) {
			if ($curr_controller == $link ['link']) {
				$link_found = 1;
				break;
			}
		}
	...
	/**
	 * Gets menu list that the user is permitted.
	 *
	 * @return result set for menu.
	 * @author rajeshkumar nadar <rajeshkumar.nadar@inscripts.com>
	 **/
	public function get_menu_list(){
		...
		$menu=implode(',',array_unique(array_filter($menuids)));
		$data=$this->ion_auth_model->menu_list_links($menu);
		$menulist=array();
		//print_r($menu);
		foreach ($data as $item) {
			if($item['parent_menu']==0)
			{
				$menulist[$item['id']]['link']=$item['link'];
				$menulist[$item['id']]['name']=$item['name'];
				$menulist[$item['id']]['icon_class']=$item['icon_class'];

			}else{
				$row['link']=$item['link'];
				$row['name']=$item['name'];
				$menulist[$item['parent_menu']]['sub_menu'][]=$row;
