...
/**
 * Ion_auth_model - Model
 *
 * @category  Model
 * @package   Models
 * @subpackage Ion_auth_model
 * @author    Dharmendra Yadav <dharmendra.yadav@inscripts.com>
 * @author    Rajeshkumar Nadar <rajeshkumar.nadar@inscripts.com>
...
class Ion_auth_model extends CI_Model
{
	 ...
	 *
	 * @author Dharmendra Yadav <dharmendra.yadav@inscripts.com>w
	 */
	...
	/**
	 * Checks If the link is present in the menu list
	 *
	 * @return bool
	 * @author Rajeshkumar Nadar <rajeshkumar.nadar@inscripts.com>
	 **/
	public function menu_list_links($menu = NULL)
	{
		if($menu=="" || $menu==NULL){
			$menu=0;
		}
		$sql="select * from menu 		
		where  id in (".($menu).") and disabled=0 order by parent_menu, order_no asc";
		//echo $sql;
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		$query = $this->db->select($this->identity_column . ', email, id, password,state_id, dist_id, taluka_id, phc_id,subcenter_id,village_id, active, last_login')
		                  ->where($this->identity_column, $identity)
		                  ->limit(1)
		    			  ->order_by('id', 'desc')
		                  ->get($this->tables['users']);
	...
	public function user($id = NULL)
	{
		...
		$id || $id = $this->session->userdata('user_id','email');
	...
	public function get_users_groups($id=FALSE)
	{
		...
		return $this->db->select($this->tables['users_groups'].'.'.$this->join['groups'].' as id, '.$this->tables['groups'].'.name, '.$this->tables['groups'].'.description,'.$this->tables['groups'].'.menu_id')
	...
	**/
	...
	public function set_session($user)
	{
		...
		$session_data = array(
		    ...
		    'phc_id'               => $user->phc_id,
		    'subcenter_id'         =>$user->subcenter_id,
	...
	**/
	...
	function getTaluka() {
		$this->db->select ( 'id, name,dist_id' );
		$this->db->where ( 'disabled', 0 );
		return $this->db->get ('taluka' )->result_array ();
	}

	function getPhc() {
		$this->db->select ( 'id, name,taluka_id' );
		$this->db->where ( 'disabled', 0 );
		return $this->db->get ('phc_center')->result_array ();
	}
}
