...
if (empty($action)) {
    ...
    if (!$DB->record_exists('customcert_issues', array('userid' => $USER->id, 'customcertid' => $customcert->id))) {
        ...
        /*Logic for certificate serial no and cvcode ******start****/
        $conn = new mysqli('localhost', 'bgzuppaczc', 'QGZAAe74UP', 'bgzuppaczc');  
        //$sql=$DB->get_record_sql("select * from {cert_serial_number}  where user_id= $USER->id");
        $sql=$DB->get_record_sql("select * from {cert_serial_number}  order by id desc limit 1");
       
        $srno=1001;
        if($sql){
            $srno=(int)substr($sql->serialnumber,1);
            $srno++;
        }
        $serial_number = new stdClass();
        $serial_number->serialnumber="e". $srno;
        $serial_number->user_id=$USER->id;
        $DB->insert_record('cert_serial_number', $serial_number);

        /*Logic for certificate serial no and cvcode ******End****/
        //update custom user fields.
        $sql="Update mdl_user_info_data set data='".$customcertissue->code."'  where userid=$USER->id and fieldid=28";
        $result = $conn->query($sql);        
        $sql="Update mdl_user_info_data set data='".$serial_number->serialnumber."'  where userid=$USER->id and fieldid=29";
        $result = $conn->query($sql);
