...
class template {
    ...
    public function generate_pdf($preview = false, $userid = null) {
        ...
        if (empty($userid)) {
            ...
            /*Logic for certificate image generation author RAJESH NADAR ******start****/
            $data=$DB->get_records('user_info_data', array('fieldid' => 27, 'userid' => $userid));
            foreach ($data as $key => $value) {
                $user->profile['profilepicture']=$value->data;
                //print_r($value->data);
            } 
            /*Logic for certificate image generation author RAJESH NADAR ******End****/
